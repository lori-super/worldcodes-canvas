...
WorldCodes 自有 npm 包发布前，优先从本地源码启动：

```bash
cd /path/to/worldcodes-canvas/canvas-agent
npm install
npm run build
node dist/index.js
```
...
```bash
claude mcp add --scope user --transport stdio infinite-canvas -- node /absolute/path/to/worldcodes-canvas/canvas-agent/dist/index.js mcp
```
...
- Claude Code SDK 说明
- Claude Agent SDK TypeScript 说明
...
- 用户从本地源码运行 `node canvas-agent/dist/index.js` 后，线上画布能显示已连接。
