...
+ [修复] 图片创作页保留已生成结果与领取错误，超时后可刷新重领或打开原图，重领不再重复生成计费。

## v0.18.0 - 2026-09-08

+ [同步] 合并上游 v0.18.0，保留 WorldCodes 渠道、品牌、同源图片重领及视频任务持久化；自有 Agent 升至 0.6.1。
+ [调整] 自定义模型脚本与 URL 自动导入凭据继续保持关闭；本地代理默认关闭，启用后使用 WorldCodes 自托管包。

+ [调整] 图片 1K / 2K / 4K 按宽高比使用固定尺寸表。
+ [调整] 开启本地代理后，WebDAV 测试连接和同步也经本机代理转发，关闭后仍直连。
+ [修复] WebDAV 同步会记录已删除画布，再次同步时不再把远端旧画布恢复到本地。
+ [新增] 配置弹窗新增「本地代理」页签，开启后模型列表、生图、生视频、生成文本、生成音频等请求经本机代理转发，配套发布 WorldCodes 自托管本地代理 纯转发工具。
...
+ [修复] PandaTK 图片经画布同源领取，生成结果链接保留到保存成功，领取失败重试沿用原结果，并显示具体下载错误。

+ [修复] 放行 PandaTK 图片结果域名的下载请求，避免生成扣费成功后图片被页面安全策略拦截。
...
+ [新增] 视频创作台支持 MiniMax H3 普通参考图、首尾帧、参考视频和参考音频，并在其他模型下阻止静默忽略不兼容素材。
+ [调整] 全站更名为 WorldCodes Canvas，移除上游站点跳转并新增站内使用文档；Agent 入口保留为可选的本地能力。
+ [调整] Codex 插件与本地 Agent 改用 WorldCodes 自有包、插件市场和正式站入口，仅保留旧 MCP ID 与本地目录作为兼容标识。
...
+ [修复] 修正 Nano Banana 2 的 Gemini 生图尺寸参数，确保比例与 1K/2K/4K 规格传入 Relay。
+ [修复] Nano Banana 2 的外链结果改经同源短时代理读取，避免生成成功后因跨域而无法写入画布。
+ [修复] 允许画布在 CSP 下读取模型返回的内嵌图片数据，避免 GPT Image 2 生成成功后无法本地保存。
+ [修复] 画布生成操作增加同步防重入，避免连续点击视频生成按钮创建重复任务并重复扣费。
+ [修复] MiniMax H3 在临时上传前校验参考图片与视频边长 256–5760px，避免上游异步失败后才返回尺寸错误。
+ [修复] 恢复 Agent 未连接页原有的插件安装、直接启动、复制命令和卸载提醒，并全部替换为 WorldCodes 自有插件与 Agent 说明。
