...
const miniMaxH3ResolutionOptions = [
...
const miniMaxH3SecondOptions = [4, 6, 8, 10, 12, 15];
const miniMaxH3RatioOptions = ["adaptive", "21:9", "16:9", "4:3", "1:1", "3:4", "9:16"];

export const videoResolutionOptions = miniMaxH3ResolutionOptions.map((item) => ({ value: item.value, label: item.label }));
export const videoSizeOptions = miniMaxH3RatioOptions.map((value) => ({ value, label: value === "adaptive" ? i18n.t("settingsPanels.video.adaptive") : value }));
export const videoSecondOptions = miniMaxH3SecondOptions.map((value) => String(value));
        ...
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title={t("settingsPanels.video.quality")} color={theme.node.muted}>
                    <div className={`grid gap-2.5 ${miniMaxH3 ? "grid-cols-4" : "grid-cols-3"}`}>
...
export function videoResolutionLabel(value: string) {
    ...
    return resolution === "2K" ? resolution : `${resolution}p`;
}

export function videoSizeLabel(value: string) {
    if (value === "adaptive" || value === "auto") return i18n.t("settingsPanels.video.adaptive");
    if (miniMaxH3RatioOptions.includes(value)) return value;
...
export function normalizeVideoResolutionValue(value: string) {
    if (value.trim().toUpperCase() === "2K") return "2K";
    if (value === "480p" || value === "low") return "480";
    if (value === "720p" || value === "auto" || value === "high" || value === "medium") return "720";
    return value.replace(/p$/i, "") || "720";
}

function isMiniMaxH3Config(config: AiConfig) {
    const activeModel = modelCapabilityOf(config, config.model) === "video" ? config.model : config.videoModel;
    return modelOptionName(activeModel).toLowerCase() === "minimax-h3";
}

function normalizeMiniMaxH3RatioValue(value: string) {
    if (value === "auto") return "adaptive";
    if (miniMaxH3RatioOptions.includes(value)) return value;
    const match = value.match(/^(\d+)x(\d+)$/);
    if (!match) return "1:1";
    const target = Number(match[1]) / Number(match[2]);
    return miniMaxH3RatioOptions.slice(1).reduce((best, ratio) => {
...
function normalizeMiniMaxH3ResolutionValue(value: string) {
