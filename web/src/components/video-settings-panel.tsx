...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    ...
    const minSeconds = profile?.min ?? VIDEO_SECONDS_MIN;
    const maxSeconds = profile?.max ?? VIDEO_SECONDS_MAX;
    const seconds = Number(profile ? normalized.videoSeconds : clampVideoSeconds(config.videoSeconds || "6"));
    const videoMode = normalizeVideoModeValue(config.videoMode);
    const resolution = profile ? normalized.vquality : parseVideoResolution(config.vquality);
    const selectedRatio = profile ? normalized.size : inferVideoRatio(config.size || "auto");
    ...
    const applySize = (nextResolution: string, ratio: string) => {
        ...
        onConfigChange("size", profile ? ratio : computeVideoSize(nextResolution, ratio));
    };
            ...
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title={t("settingsPanels.video.quality")} color={theme.node.muted}>
                    <div className="grid grid-cols-4 gap-2.5">
                        {(profile ? profile.resolutions.map(value => ({ value, label: value })) : resolutionOptions).map((item) => (
                            <OptionPill key={item.value} selected={resolution === item.value} theme={theme} onClick={() => selectResolution(item.value)}>
                                {item.label}
                            </OptionPill>
                        ))}
                        {profile ? null : <ResolutionInput value={resolution} theme={theme} onChange={selectResolution} />}
                    </div>
                </SettingGroup>
                {profile ? null : <SettingGroup title={t("settingsPanels.video.size")} color={theme.node.muted}>
                ...
                <SettingGroup title={t("settingsPanels.video.ratio")} color={theme.node.muted}>
                    <div className="grid grid-cols-4 gap-2.5">
                        {videoRatioOptions.filter(item => !profile || item.value !== "auto").map((item) => (
                            <button
                                key={item.value}
                                type="button"
                ...
                <SettingGroup title={t("settingsPanels.video.seconds")} color={theme.node.muted}>
                    <div className="flex items-center gap-3" onMouseDown={(event) => event.stopPropagation()}>
                        ...
                        <SecondsInput min={minSeconds} max={maxSeconds} value={seconds} theme={theme} onCommit={(value) => onConfigChange("videoSeconds", String(value))} />
                        <span className="shrink-0 text-sm" style={{ color: theme.node.muted }}>s</span>
                    </div>
                </SettingGroup>
                {profile ? null : <SettingGroup title={t("settingsPanels.video.mode")} color={theme.node.muted}>
...
export function videoResolutionLabel(value: string) {
    return /^(?:\d+p|2k)$/i.test(value) ? value : `${parseVideoResolution(value)}p`;
}
...
function SecondsInput({ value, theme, onCommit, min, max }: { min: number; max: number; value: number; theme: CanvasTheme; onCommit: (value: number) => void }) {
    const commit = (input: HTMLInputElement) => {
        const next = Math.max(min, Math.min(max, Math.round(Number(input.value) || min)));
