...
export function CanvasNodeHoverToolbar({
    ...
    const canRetry = node.metadata?.status === "error" && !(isVideo && Boolean(node.metadata?.videoTask?.id || node.metadata?.videoTaskId) && !hasVideo);
    const canQueryVideoTask = isVideo && Boolean(node.metadata?.videoTask?.id || node.metadata?.videoTaskId) && !hasVideo && node.metadata?.status !== "loading";
                ...
                <div className="h-[56vh] min-h-[360px] select-text text-sm" data-canvas-shortcuts-ignore>
                    {view === "info" ? (
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            {node.metadata?.videoTask?.id || node.metadata?.videoTaskId ? <InfoRow label={t("canvas.nodeToolbar.videoTaskId")} value={node.metadata.videoTask?.id || node.metadata.videoTaskId || ""} /> : null}
