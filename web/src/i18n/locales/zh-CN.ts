export default {
    ...
    theme: { toggle: "切换主题" },
    apiReferenceDimensions: { image: "MiniMax H3 参考图每条边须为 256–5760 px（当前 {{width}}×{{height}}）", video: "MiniMax H3 参考视频每条边须为 256–5760 px（当前 {{width}}×{{height}}）" },
    common: {
        cancel: "取消",
        save: "保存",
        edit: "编辑",
    ...
    videoWorkbench: {
        ...
        referenceLimit: "参考素材数量已达当前类型或模型上限",
        referenceUploadFailed: "参考素材读取失败，请重新上传",
        imageRole: "参考图角色",
        roleReference: "普通参考图",
        roleFirstFrame: "首帧",
        roleLastFrame: "尾帧",
        h3ReferencesOnly: "参考视频、参考音频和首尾帧角色仅支持 MiniMax H3，请切换模型或移除这些素材",
        standardImageLimit: "当前视频模型最多支持 7 张普通参考图",
        clipboardEmpty: "剪切板里没有可读取的图片",
        clipboardAdded: "已读取 {{count}} 张参考图",
    ...
    canvas: {
        ...
        export: { defaultProjectName: "WorldCodes Canvas", defaultNodesName: "画布元素", item: "元素" },
        createMenu: {
    ...
    home: {
        promptError: "获取提示词失败",
        description: "在 <canvas>WorldCodes Canvas</canvas> 中生成、连接和重组 <content>图片、文字与图形</content>，让创作从单次生成变成连续推演。",
        start: "开始使用",
        openCanvas: "打开画布",
    ...
    config: {
        ...
        localStorage: {
            ...
            description: "查看 WorldCodes Canvas 在浏览器中保存的数据量，并按对象仓库统计内容体积。",
            ...
            mainDatabase: "WorldCodes Canvas 主数据",
            records: "{{count}} 条",
            refresh: "刷新统计",
            readFailed: "读取本地存储失败",
        ...
        export: "导出配置",
        imported: "配置与用户偏好已导入",
        importedDirectConfig: "已导入渠道配置",
        importFailed: "配置文件读取失败",
        saved: "配置已保存",
        savedContinue: "配置已保存，请继续刚才的请求",
        channels: {
            description: "为渠道选择协议和模型，并为每个模型指定生图、视频、文本或音频能力。WorldCodes 生产站默认使用同源 Relay。",
            add: "新增渠道",
            unnamed: "未命名渠道",
        ...
        channelEditor: {
            ...
            baseUrl: "接口地址",
            models: "渠道模型",
            modelDescription: "已选 {{count}} 个；为每个模型指定能力。",
            selectModels: "同步/选择模型",
            syncModels: "同步可用模型",
        ...
        modelSelect: {
            ...
            add: "增加模型",
            fetch: "拉取模型列表",
            description: "模型来自当前渠道的 /v1/models。WorldCodes Token 会按 Auto 分组与模型限制返回可用范围；不提供该接口时可手动增加。",
    ...
    agent: {
        ...
        connect: { pluginTitle: "方式一：使用 WorldCodes Codex 插件", pluginText: "安装 WorldCodes Canvas 自有插件后，在新的 Codex 任务中输入“打开 WorldCodes Canvas”。插件会启动自有 MCP，并按需启动本地 Agent、打开正式站和自动带入连接信息。", directTitle: "方式二：直接运行 WorldCodes Agent", directText: "不使用 Codex 插件时，在终端运行下面的 WorldCodes 自托管 Agent，再回到网页连接；Local URL 和 Connect token 通常会自动发现。", commandCopied: "命令已复制", pluginReminder: "WorldCodes 插件提醒", pluginReminderText: "安装插件会把 WorldCodes Canvas 工具加入 Codex 上下文；仅直接运行 Agent 只提供网页连接和对话，不会安装 MCP。生图、视频生成和素材管理不依赖 Agent。", removePlugin: "移除插件", removeMcp: "移除旧手动 MCP", copyCommand: "复制命令", title: "连接 WorldCodes Agent", description: "按使用场景选择一种连接方式。", webConnection: "网页连接", autoDiscover: "默认自动读取 Local URL 和 Connect token，失败时再手动填写。", disconnect: "断开", connect: "连接", localAddress: "本地地址", urlPlaceholder: "例如 http://127.0.0.1:17371", token: "连接 Token", tokenPlaceholder: "自动发现，或手动填入 Connect token" },
        ...
        events: { analyzing: "正在分析任务…", reasoning: "思考摘要", plan: "执行计划", commandFailed: "命令执行失败", commandCompleted: "命令已完成", executeCommand: "执行命令", editFiles: "修改文件", searchWeb: "搜索资料", viewImage: "查看图片", imageViewed: "已查看图片", viewingImage: "正在查看图片", imageGeneration: "内置生图", imageFailed: "图片生成失败", imageCompleted: "图片生成完成", generatingImage: "正在生成图片…", compactContext: "整理上下文", contextCompacted: "已整理当前对话，继续处理任务", compactingContext: "正在整理当前对话…", collaboration: "协作处理", collaborationFailed: "协作任务失败", collaborationCompleted: "已完成协作任务", collaborating: "正在协作处理任务…", progress: "任务进度", progressCount: "已完成 {{completed}}/{{total}} 项", planning: "正在整理执行步骤…", executingCommand: "正在执行命令…", workingDirectory: "工作目录", exitStatus: "退出状态", duration: "耗时", seconds: "{{value}} 秒", filesCompleted: "已完成文件修改", preparingFiles: "正在准备文件修改…", fileCompleted: "已{{action}} {{path}}", editingFile: "正在{{action}} {{path}}", filesEdited: "已修改 {{count}} 个文件：{{names}}{{more}}", editingFiles: "正在修改 {{count}} 个文件：{{names}}{{more}}", andMore: " 等", openPage: "打开网页：{{url}}", findInPage: "在网页中查找“{{pattern}}”", content: "内容", search: "搜索：{{query}}", relatedInfo: "相关资料", keyword: "关键词", webpage: "网页", add: "新增", delete: "删除", edit: "修改", diagnostics: "WorldCodes Canvas Agent 诊断", address: "地址：{{endpoint}}", connection: "连接：{{connection}} · 状态：{{status}}", online: "在线", connecting: "连接中", disabled: "未启用", messageTool: "消息：{{messages}} · 工具：{{tool}}", none: "无", noLogs: "暂无事件日志", threadCreated: "创建会话", turnStarted: "开始处理", progressUpdated: "更新任务进度", turnFailed: "处理失败", turnStopped: "处理已停止", turnCompleted: "处理完成", toolCalled: "调用工具", toolFailed: "工具失败", toolCompleted: "工具已完成", replyReceived: "收到回复", completed: "完成", modelBusy: "模型暂时繁忙", modelBusyDescription: "当前选择的模型请求量过大，暂时无法处理。请稍后重试，或切换其他模型后再试。", taskFailed: "任务失败", taskFailedDescription: "Codex 未能完成本次任务，请稍后重试。" },
