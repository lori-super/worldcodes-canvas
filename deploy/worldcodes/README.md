...
CSP 允许请求同源 API、WorldCodes API、内嵌 `data:` 内容、一个精确 R2 S3 Origin，以及 PandaTK 图片结果域 `https://download.xmimage2.cc.cd`。画布用 `fetch` 下载结果后写入本地，因此结果域必须列入 `connect-src`，仅允许 `img-src https:` 不足以完成交付。内置提示词 JSON 已随站点静态包发布，不再请求 GitHub。未放行 `unsafe-eval`、任意模型 API Origin、远程模型脚本、远程插件、WebDAV 或统计脚本。当前主题初始化仍是内联脚本，因此暂时保留 `script-src 'unsafe-inline'`。

Wanmo 边缘实例使用 `/etc/caddy/Caddyfile.worldcodes-edge` 的 Canvas 站点块。维护该实例时，只更新该站点的对应 CSP 源，保留现有 API 转发、R2 与媒体域配置；先备份并校验完整 Caddy 配置，再原子替换并 reload，失败恢复备份。

结果域回归已在真实线上页面验证：修复前 `fetch` 触发 `securitypolicyviolation`，指令为 `connect-src`；修复后请求到达结果服务，未再触发该事件，未列入白名单的测试域仍被阻止。本次复用了早先生成的测试链接，该链接已返回 404，所以此项验证只证明域名策略已修复，不代表新的完整生图验收或历史图片找回。没有提交付费生成。完整生产 Caddy 候选配置校验通过；本机部署总检查因缺少 Caddy 2.10.2 未完成。
