# @worldcodes/canvas-proxy

WorldCodes Canvas 的本地转发代理。浏览器直连第三方 AI 接口时经常被 CORS 拦截，启动它之后，网页会把请求先发到本机，再由本机转发到目标地址。
...
```bash
npx https://canvas.worldcodes.online/downloads/worldcodes-canvas-proxy-0.1.0.tgz
```

默认监听 `http://127.0.0.1:23210`。把这个地址填进 WorldCodes Canvas 的「配置 → 本地代理」，并打开开关即可。
...
```bash
npx https://canvas.worldcodes.online/downloads/worldcodes-canvas-proxy-0.1.0.tgz --port 23210 --host 127.0.0.1
```
